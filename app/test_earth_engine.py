...
# def get_soil_moisture(latitude: float, longitude: float):
    ...
    # Use a wider range while testing
    start_date = (now - timedelta(days=7)).strftime("%Y-%m-%d")
    end_date = (now + timedelta(days=1)).strftime("%Y-%m-%d")

    # ---------------------------------
    # Build all the images up front. This is local/free -- no network
    # calls happen until .getInfo() is called below, so building
    # everything first costs nothing and lets us fire the independent
    # reduceRegion calls in parallel threads.
    # ---------------------------------
    ...
    # ORGANIC CARBON + PH + WATER + TEXTURE -- all scale 250, so they
    # are merged into ONE image and fetched with ONE reduceRegion call
    # instead of the two (carbon/ph/water, then texture) the previous
    # version made.
    soc_image = ee.Image("projects/soilgrids-isric/soc_mean")
    carbon = soc_image.select(
        "soc_0-5cm_mean").multiply(0.1).rename("organic_carbon_g_kg")
    ...
    # NOTE: the original code read water content from
    # ee.ImageCollection("ISRIC/SoilGrids250m/v2_0").first(), guarded by
    # a separate .size().getInfo() call purely to check the collection
    # wasn't empty. That guard is a full extra network round-trip and
    # buys nothing -- if the collection were empty, .first() still
    # returns a (fully masked) image, and reduceRegion already comes
    # back with None for masked bands, which downstream code already
    # handles. So the guard call is dropped here.
    water_collection = ee.ImageCollection("ISRIC/SoilGrids250m/v2_0")
    first_water_image = ee.Image(water_collection.first())
    water = first_water_image.select("val_0_5cm_mean").multiply(
        0.001).rename("water_content_volume_fraction")
    ...
    properties_and_texture = ee.Image.cat([carbon, ph, water])

    # SMAP -- scale 9000, needs its own date-filtered collection
    collection = (
        ee.ImageCollection("NASA/SMAP/SPL4SMGP/008")
        .filterDate(start_date, end_date)
        .sort("system:time_start", False)
    ...
    # ---------------------------------
    # Fire the three independent lookups concurrently. Each .getInfo()
    # call is a blocking network request; running them on separate
    # threads means we pay for the slowest one instead of the sum of
    # all three (plus whatever their individual buffer-radii fallbacks
    # need).
    # ---------------------------------
    ...
    def fetch_properties():
        return _reduce_with_fallback(properties_and_texture, point, scale=250)

    def fetch_smap():
        # NOTE: the previous version called collection.size().getInfo()
        # here purely to check the collection wasn't empty before doing
        # the real fetch -- an extra, fully sequential network round
        # trip. The same reasoning already applied to the water-content
        # guard above applies here: if the collection is empty,
        # .first() returns a fully-masked image, reduceRegion returns
        # None for every band, and the `smap_values` all-None case is
        # already handled below the same way an empty collection was
        # handled before (via the null checks in analyze_soil_condition
        # and the farm_health_score confidence logic). So the size()
        # pre-check is dropped -- same outcome, one fewer round trip.
        image = ee.Image(collection.first())
        selected_bands = image.select([
            "sm_surface",
            "sm_rootzone",
    ...
    # If every SMAP band came back null (e.g. no imagery for the date
    # range, same case that previously surfaced as an empty
    # collection), report it exactly as before rather than silently
    # continuing with all-None values.
    if not any(v is not None for v in smap_values.values()):
        return {
            "success": False,
            "message": "No soil moisture images found for the requested date range",
    ...
    # ---------------------------------
    # Assemble results (identical shape to the original function)
    # ---------------------------------

    nutrient = {
        "nitrogen": (
            nutrient_values.get("nitrogen_g_kg") * 1000
            if nutrient_values.get("nitrogen_g_kg") is not None
            else None
        ),
        "phosphorus": nutrient_values.get("phosphorus_mg_kg"),
        "potassium": nutrient_values.get("potassium_mg_kg"),
    ...
    organicComp = {
        "organic_carbon": {
            "value": values.get("organic_carbon_g_kg"),
            "unit": "g/kg",
        },
        "ph": {
            "value": values.get("soil_ph"),
            "unit": "pH",
        },
        "water_content_at_field_capacity": {
            "value": values.get("water_content_volume_fraction"),
    ...
    clay_value = values.get("clay_pct")
    sand_value = values.get("sand_pct")
    texture_result = classify_texture_bucket(sand_value, clay_value)

    texture_info = {
        "usda_class": texture_result["usda_class"],
        "texture_class": texture_result["model_bucket"],
        "clay_pct": clay_value,
        "sand_pct": sand_value,
        "source": "ISRIC SoilGrids",
        "sample_radius_m": soil_radius,
